...
- **SIM-05 用途边界**：输出用于默认参数选型、Stage 4 验收锚点（实测与预测偏差 ≤ 2×）和 Stage 5 延迟扫描测点；不得作为运行时正确性依据（INV-10 同理适用）。
...
- **BENCH-05（可选）对象存储**：同套基准在 S3 类存储上执行，原子 PUT / 条件写替代 rename，作为 STOR-01 可实例化的旁证。
...
| A-BENCH-04 | M 并发写一轮 fragments 的时间 ≤ 10% 同步周期 |
...
| A-PERF-04 | GPU goodput ≥ 95%（对照关闭通信的本地吞吐；160M 与 0.5–1B 各一次） | 吞吐对照报告 |
| A-PERF-05 | 快照+采用暂停合计 ≤ 2% 步时间 | timing trace |
| A-PERF-06 | 端到端延迟可分解 | 快照/写/发现/读/采用分解报告 |
...
- **STALE-04 正确 displacement**：`g_{i,f} = G_f^b - L_{i,f}`；不得使用 `G_f^v - L_{i,f}`（INV-07；Stage 0-C 的 s=1 golden cases 在此启用为运行时测试）。
...
**消融配置要求**：所有消融两臂共享同一二进制（DISC-03），轴为 `S_max ∈ {0,1}`、`lambda_s ∈ {0.5,1,2}`、`Q_fresh ∈ {1,0}`、异构比 `∈ {1.0,1.5,2,3}`；质量口径 matched-token。
...
| A-STALE-02 | `s=1` 使用 old base displacement | 反例向量 + 运行时 trace |
...
| A-STALE-07 | stale 接受率与仿真预测一致 | 接受率 > 5% 且与 Stage 0-A 预测偏差 ≤ 2× |
...
| A-ALG-02 | stale-aware 与 fresh-only 有独立 matched 结果 | matched-token 实验报告（H3 检验见 RESEARCH_PLAN Stage 4） |

**质量 gate 与回退**：H3 matched-token 检验（阈值见 RESEARCH_PLAN Stage 4）失败时，默认运行配置回退 `S_max = 0`；机制与测试保留，结果作为负面结果报告。
...
- **TEL-02 syncer 指标**：proposal discovery latency；quorum wait；grace wait；selected learner count；fresh/stale count；staleness histogram；normalized weights；rejection reason counts（按 STALE-10 分类）；payload read bytes/time；merge time；outer optimizer time；publication time；per-fragment update interval；ready queue age。
...
- **REPORT-01 stale 结论必报量**：`S_max`、`lambda_s`、`Q`、`Q_fresh`、grace window、accepted staleness distribution、fresh/stale normalized weight mass、rejected-too-stale 比例、learner adoption lag、fragment update frequency、total bytes/token、tokens/FLOPs/wall-clock 预算、fresh-only 对照、direct averaging 对照、loss 与 evaluation、GPU goodput 与 syncer/存储成本。
...
| 0-B | A-BENCH-01..04 |
| 0-C | A-ALG-00 |
| 1 | A-FRAG-01..03、A-PROP-01..03、A-GLOBAL-01..02、A-LEARN-01..03、A-PERF-01、A-ALG-01、A-EVAL-01 |
...
| 故障恢复 | kill-restart 级别纳入 Stage 3 验收；不做 exact replay 与自动 failover |
