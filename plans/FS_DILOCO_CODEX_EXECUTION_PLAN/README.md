...
7. 基础训练系统在 `S1-13` 完成后，后续每个实现 loop 都必须以一次新的真实 Miyabi 9 节点门禁结束：8 learners + 1 syncer、每 learner 1 GPU/1 node、`H=50` local optimizer steps、`target_global_cycles=10`，loss 有限且呈下降趋势，运行时间在预先冻结的预算内。
