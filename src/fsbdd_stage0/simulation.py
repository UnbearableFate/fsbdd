...
class SimulationConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.tokens_per_step <= 0 or self.max_trace_events < 0:
            raise OracleInputError("tokens_per_step must be positive and trace cap non-negative")
        numeric = {
        ...
        if self.duration_seconds <= 0 or self.heterogeneity_ratio < 1:
            raise OracleInputError("duration must be positive and heterogeneity_ratio >= 1")
    ...
    def offsets(self) -> tuple[int, ...]:
        return tuple((fragment * self.h_steps) // self.fragments for fragment in range(self.fragments))
...
class _Simulator:
    ...
    def _handle_step(self, time_value: float, learner: int) -> None:
        ...
        for fragment in range(self.config.fragments):
            ...
            proposal = Proposal(
                ...
                local_steps=max(1, self.tokens_since_adopt[learner][fragment] // self.config.tokens_per_step),
    ...
    def _perform_update(self, time_value: float, fragment: int, result) -> None:
        ...
        stale = [proposal for proposal in selected if proposal.base_version < current_version]
    ...
    def run(self) -> SimulationResult:
        ...
        accepted_efficiency = self.accepted_tokens / opportunities if opportunities else 0.0
