...
from .storage import PosixStorageBackend, PublicationInterrupted
from .syncer_readiness import FrozenSelection
...
def _load_config(path: Path, expected_identity: str) -> dict[str, Any]:
    ...
    if (
        ...
        or value["s_max"] != 0
        or selection
        != {"q": 4, "q_fresh": 4, "max_contributors": 4, "lambda_s": 1.0}
        or set(policy)
        != {
            "merge",
            "outer_optimizer",
        ...
        raise GlobalCommitStressError("global commit profile differs from frozen Stage 1 values")
    return value
    ...
    for crash_at in config["publication_fault_points"]:
        for replay in range(repeats):
            ...
            try:
                ...
                raise GlobalCommitStressError("fault injection did not interrupt publication")
    ...
    for cycle in range(cycles):
        ...
        for fragment_index in range(len(atomic.store.descriptors)):
            ...
            if len(candidates) != len(atomic.learner_ids):
                raise GlobalCommitStressError("committer did not observe complete proposal quorum")
...
def _validate_authority_summary(value: Any) -> dict[str, Any]:
    ...
    semantic = {key: item for key, item in value.items() if key != "authority_fingerprint"}
...
def analyze_roles(
    ...
    if writer.get("role") != "proposal_writer_reader" or committer.get("role") != "atomic_committer":
    ...
    for role, rank in ((writer, 1), (committer, 0)):
        if (
            ...
            raise GlobalCommitStressError("formal role identity or CPU contract differs")
    ...
    for fragment, rows in per_fragment.items():
        if [row[1]["version"] for row in rows] != list(range(1, cycles + 1)):
            raise GlobalCommitStressError(f"fragment {fragment} version trace is not exact +1")
        for previous, current in zip(rows, rows[1:], strict=False):
            if previous[1]["authority_fingerprint"] != current[0]["authority_fingerprint"]:
                raise GlobalCommitStressError("successive authority chain is discontinuous")
    ...
    for trace in fault_traces:
        ...
        if point == "after_record_replace":
            if (
                ...
                raise GlobalCommitStressError("post-replace replay classification differs")
        elif (
    ...
    expected_visibility = [f"global-current-{index:06d}.json" for index in range(fragments)]
    if (
        committer.get("global_visibility_records") != expected_visibility
        or committer.get("shared_global_head_present") is not False
    ):
        raise GlobalCommitStressError("fixed per-fragment current record contract differs")
    return {
        "schema_version": 1,
        "status": "pass",
