...
from .learner_smoke import _load_real_model, _parameter_digest
...
def _copy_validated_dataset(profile: Any, source: Path, destination: Path) -> dict[str, Any]:
    ...
    *,
    ...
    if reuse is not None:
        ...
        if manifest.get("split") != split:
            raise Stage1AssetError("reused dataset split differs from the requested split")
        return manifest
    ...
    *,
    ...
    try:
        ...
        manifest = {
            ...
            "profiles": {
                "nine_node": {"path": str(gpt_profile_path), "digest": gpt_profile.digest},
                "long_run": {"path": str(pythia_profile_path), "digest": pythia_profile.digest},
            },
            "datasets": {
                "gpt2_train": _dataset_summary(dataset_root / "gpt2-train", gpt_train),
                "gpt2_validation": _dataset_summary(dataset_root / "gpt2-validation", gpt_validation),
                "pythia_smoke": _dataset_summary(dataset_root / "pythia-smoke", pythia_smoke),
                "pythia_long": _dataset_summary(dataset_root / "pythia-long", pythia_long),
            },
            "workloads": {
                "nine_node": {
                    "root": str((temporary / "workloads" / "nine_node").resolve()),
                    "manifest_sha256": _hash_file(temporary / "workloads" / "nine_node" / "manifest.json"),
                    "bootstrap_identity": nine_bootstrap["bootstrap_identity"],
                },
                "long_run": {
                    "root": str((temporary / "workloads" / "long_run").resolve()),
                    "manifest_sha256": _hash_file(temporary / "workloads" / "long_run" / "manifest.json"),
        ...
        for workload, bootstrap in (("nine_node", nine_bootstrap), ("long_run", long_bootstrap)):
            resolved = json.loads(json.dumps(config))
            resolved["resolved_runtime_fields"] = {
                "fragment_descriptors": [row["descriptor"] for row in bootstrap["fragments"]],
                "fragment_bytes": bootstrap["fragment_bytes"],
                "fragment_offsets": bootstrap["per_learner_schedules"][0]["offsets"],
                "per_learner_offsets": [row["offsets"] for row in bootstrap["per_learner_schedules"]],
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Prepare immutable S1-13 model/data/bootstrap assets")
