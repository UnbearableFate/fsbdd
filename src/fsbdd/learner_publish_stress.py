    ...
    *,
    ...
    writer = {
        ...
        "training": {
            ...
            "safe_boundary_steps": [
                event.local_optimizer_step for event in run.events
            ],
...
def _reader(
    *,
    ...
    bases = {
        int(value["fragment_index"]): value for value in catalog["base_states"]
    }
    observed: dict[int, list[int]] = {descriptor.index: [] for descriptor in descriptors}
    polls = 0
    ...
    while not (coordination / "writer-complete.json").exists():
        ...
        for descriptor in descriptors:
            try:
                ...
                continue
            if not observed[descriptor.index] or observed[descriptor.index][-1] != proposal.sequence:
                if observed[descriptor.index] and proposal.sequence < observed[descriptor.index][-1]:
                    raise SnapshotStressError("reader observed a regressing latest sequence")
    ...
    *,
    ...
    summary = {
        ...
        "backpressure": {
            "pending_bound": 1,
            "in_flight_bound": 1,
            "maximum_pending_per_fragment": publication[
                "maximum_pending_per_fragment"
            ],
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m fsbdd.learner_publish_stress")
