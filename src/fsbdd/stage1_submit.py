...
def _marker(
    *,
    ...
    return {
        ...
        "asset_marker_sha256": _identity(
            asset_marker_sha256, "asset_marker_sha256"
        ),
        "gate_contract_sha256": _identity(
...
def validate_nine_node_roots(
    *,
    ...
    expected_sha256 = _identity(
        submission_marker_sha256, "submission_marker_sha256"
    )
    for root in (shared, result):
        marker = root / "submission-root.json"
        if not marker.is_file() or marker.is_symlink():
            raise Stage1SubmissionError(f"submission marker is absent or unsafe: {marker}")
        if _sha256(marker) != expected_sha256:
            raise Stage1SubmissionError(f"submission marker checksum mismatch: {marker}")
        try:
            actual = json.loads(marker.read_text(encoding="utf-8"))
        except json.JSONDecodeError as error:
            raise Stage1SubmissionError(f"invalid submission marker: {marker}") from error
        if actual != expected:
            raise Stage1SubmissionError(f"submission marker identity mismatch: {marker}")
