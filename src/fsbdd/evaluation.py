...
class FrozenEvaluationPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or tuple(
                item.state.descriptor.index for item in self.authorities
            )
    ...
    def capture(
        ...
        *,
        ...
        if not isinstance(store, AtomicGlobalCommitStore):
            raise EvaluationSnapshotError(
                "capture requires AtomicGlobalCommitStore"
            )
    ...
    *,
    ...
    for record in frozen_fragments:
        ...
        if (
            ...
            or hashlib.sha256(state.parameters).hexdigest()
