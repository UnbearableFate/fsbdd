...
def _robust_lag_slope(values: Sequence[float], minimum_lag: int) -> float:
    ...
    slopes = [
        (values[index + lag] - values[index]) / lag
...
def _loss_gate(
    ...
    *,
    ...
    for role in sorted(roles, key=lambda item: int(item["learner_index"])):
        ...
        target_tokens = [
            int(item["loss_bearing_target_tokens_step"]) for item in events
        ]
    ...
    if workload == "nine_node":
        ...
        validation_pass = (
            ...
            and all(
                int(validation[key]["packed_blocks"]) == 487
...
def _topology_gate(
    ...
    *,
    ...
    job_ids = [str(item["identity"]["pbs_job_id"]) for item in roles] + [
        str(syncer["identity"]["pbs_job_id"])
    ]
    ...
    asset_identities = {
        str(item["identity"]["asset_marker_sha256"]) for item in roles
    ...
    passed = (
        ...
        and len(
            {int(item["identity"]["shared_device"]) for item in roles}
            | {int(syncer["identity"]["shared_device"])}
        )
        == 1
    )
    return {
        ...
        "shared_filesystem_device_ids": sorted(
            {int(item["identity"]["shared_device"]) for item in roles}
            | {int(syncer["identity"]["shared_device"])}
        ),
        "syncer_cpu_only": (
...
def _runtime_gate(
    ...
    *,
    ...
    walltime_limit = float(
        gate_contract["runtime"]["active_runtime_must_be_below_pbs_walltime_fraction"]
    )
    target = int(
        execution_contract.get(
    ...
    for role in roles:
        events = _records(
            Path(role["_result_root"]) / "logs" / f"{role['learner_id']}.jsonl",
            "safe_boundary",
        )
    ...
    for index, (left, right) in enumerate(zip(completion_times, completion_times[1:])):
        gap = (right - left) / 1e9
        previous_cycle = int(target_updates[index]["global_cycle_after"])
        cycle_before_previous = (
            -1 if index == 0 else int(target_updates[index - 1]["global_cycle_after"])
        )
        if (
    ...
    for item in target_updates:
        per_fragment_completion_times.setdefault(
            int(item["fragment_index"]), []
        ).append(int(item["completed_unix_ns"]))
    ...
    for role in roles:
        ...
        row_pass = (
            ...
            and all(
                int(item) <= 1 for item in publication["maximum_pending_per_fragment"]
            )
            and all(
                int(item) <= 1 for item in publication["maximum_in_flight_per_fragment"]
            )
    ...
    return {
        ...
        "pending_stall": {
            ...
            "syncer": {
                ...
                "resident_selected_proposals": readiness["resident_selected_proposals"],
                "resident_latest_proposals": readiness["resident_latest_proposals"],
...
def _protocol_gate(
    ...
    *,
    ...
    contiguous = all(
        values == list(range(1, len(values) + 1)) for values in per_fragment.values()
    )
    global_cycle = int(syncer["progress"]["global_cycle"])
    recomputed_global_cycle = min(
        (len(values) for values in per_fragment.values()), default=0
    )
    target = int(
        execution_contract.get(
            "target_global_cycles", execution_contract.get("minimum_global_cycles")
    ...
    inventory_schedule_pass = set(range(interval, global_cycle + 1, interval)).issubset(
        periodic_cycles
    )
    reclamation_pass = workload == "nine_node" or all(
        any(
            int(item[name].get("reclaimed_payload_files", 0)) > 0
            for item in inventories
        )
        for name in expected_visibility
    )
    inventory_pass = (
        classified_inventory_pass and inventory_schedule_pass and reclamation_pass
    )
    forbidden_pass = (
        all(
            not bool(item["forbidden_runtime"]["torch_distributed_initialized"])
            for item in roles
        )
    ...
    schedule_pass = all(
        item["publication"]["schedule"]["fragment_bytes"] == resolved["fragment_bytes"]
    ...
    opportunity_pass = workload != "long_run" or (
        ...
        and (
            ...
            or global_cycle / minimum_opportunities >= float(minimum_cycle_ratio)
        )
    )
    passed = (
        len(per_fragment) == 4
    ...
    return {
        ...
        "fragment_version_sequences": {
            str(key): value for key, value in sorted(per_fragment.items())
        },
...
def analyze(
    ...
    *,
    ...
    if (
        gate_contract.get("loop_id") != "S1-13"
        or gate_contract.get("schema_version") != 1
    ):
    ...
    roles = [
        _read(result_root / "roles" / f"learner-{index:02d}.json")
    ...
    passed = all(
        item["status"] == "pass" for item in (topology, loss, runtime, protocol)
    )
    summary = {
        "schema_version": 1,
        "status": "pass" if passed else "fail",
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Analyze S1-13 topology/loss/runtime/protocol gates"
    )
