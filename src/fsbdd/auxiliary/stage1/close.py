...
from fsbdd.diloco.protocol.proposal import ProposalStore
...
def _wait_roles(
    root: Path, prefix: str, count: int, timeout_seconds: float
) -> list[dict[str, Any]]:
    deadline = time.monotonic() + timeout_seconds
    while True:
...
def _stores(
    ...
    *,
    ...
    AtomicGlobalCommitStore, ProposalStore, PosixStorageBackend, PosixStorageBackend
]:
    descriptors = tuple(
        _descriptor(row["descriptor"]) for row in bootstrap["fragments"]
    )
...
def _bootstrap(
    ...
    if result.snapshot.version_vector != tuple(0 for _ in fragments):
        raise Stage1CloseError(
            "bootstrap did not produce the version-zero authority vector"
        )
...
def _apply_fragment_payloads(
    groups: Sequence[Sequence[Any]], payloads: Sequence[bytes]
) -> None:
    import torch
        ...
        for group, payload in zip(groups, payloads, strict=True):
            ...
            for parameter in group:
                ...
                source = torch.from_numpy(
                    array[offset : offset + count].copy()
            ...
            if offset != array.size:
                raise Stage1CloseError(
                    "fragment payload does not cover its model group"
                )
    ...
    with torch.no_grad():
        for start in range(0, total_blocks, batch_size):
            ...
            for block_index in range(start, min(total_blocks, start + batch_size)):
                ...
                blocks.append(
                    np.asarray(arrays[shard_index][local_index], dtype=np.int64)
                )
...
def run_learner(
    *,
    ...
    learner_count = int(
        workload_config["learner_count"]
        if learner_count_override is None
        else learner_count_override
    )
    ...
    if workload == "long_run":
        ...
        if (
            phase_overlay.get("algorithm") != "aligned_zero_for_q_equals_m_capacity_v1"
    ...
    if workload == "nine_node" and learner_index == 0:
        ...
        _apply_fragment_payloads(
            groups, [item.parameters for item in loaded.authorities]
        )
        validation["initial"] = {
    ...
    _replace_json(
        shared_root / "coordination" / f"ready-{learner_index:02d}.json", ready
    )
    ...
    runtime = LearnerRuntime(
        ...
        max_grad_norm=float(
            config["training"]["inner_optimizer"]["gradient_clip_norm"]
        ),
        ...
        stop_requested=(
            (
                lambda: (
                    ...
                    and progress.local_optimizer_steps >= minimum_points
                )
            )
            if workload == "nine_node"
            else None
    ...
    _replace_json(
        shared_root / "coordination" / f"final-{learner_index:02d}.json", result
    )
    return result
...
def _update_record(update: Any, global_cycle: int, completed_ns: int) -> dict[str, Any]:
    return {
        ...
        "proposal_processed_tokens": [
            item.processed_tokens for item in update.selection.proposals
        ],
...
def run_syncer(
    *,
    ...
    if os.environ.get("CUDA_VISIBLE_DEVICES") not in {"", "-1"}:
        raise Stage1CloseError(
            "syncer requires CUDA_VISIBLE_DEVICES to disable accelerators"
        )
    config, asset_root, _asset_manifest, bootstrap = _load_contract(
        config_path,
        expected_config_sha256=config_sha256,
    ...
    if workload == "nine_node":
        materialize_evaluation_snapshot(
            initial_plan, shared_root / "evaluation" / "initial"
        )
    bootstrap_record = {
    ...
    while True:
        ...
        while True:
            ...
            update = executor.execute_next(
                observed_ns=time.monotonic_ns(), poll_store=False
            )
            if update is None:
                break
            made_progress = True
            ...
            inventory = {
                ...
                "global": global_backend.reclaim_unreferenced_payloads(
                    ...
                    minimum_age_seconds=float(
                        config["bounded_state"][
                            "minimum_unreferenced_payload_age_seconds"
                        ]
                    ),
                ),
        ...
        if workload == "nine_node" and cycle >= target:
            ...
            final_manifest = materialize_evaluation_snapshot(
                final_plan, shared_root / "evaluation" / "final"
            )
            completion = {
                "complete": True,
                "run_id": run_id,
            ...
            final_records = _wait_roles(
                shared_root / "coordination", "final", learner_count, timeout_seconds
            )
            break
        if workload == "long_run":
            paths = [
            ...
            if (
                ...
                _replace_json(
                    shared_root / "coordination" / "complete.json", completion
                )
                break
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the S1-13 filesystem-only closure workload"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if arguments.role == "learner":
        if arguments.learner_index is None or arguments.hub_cache is None:
            raise Stage1CloseError(
                "learner role requires --learner-index and --hub-cache"
            )
        result = run_learner(
    ...
    print(
        json.dumps(
            {"status": result["status"], "workload": result["workload"]}, sort_keys=True
        )
    )
    return 0
