...
from fsbdd.diloco.model.fragment_map import build_fragment_map, partition_layer_bytes
...
def analyze() -> dict[str, Any]:
    ...
    for _ in range(random_cases):
        ...
        if actual != expected:
            raise AssertionError(
                f"random mismatch weights={weights} F={fragment_count}: {actual} != {expected}"
            )
    return {
        "schema_version": 1,
        "status": "pass",
    ...
    for seed in ("1", "2", "7", "31"):
        ...
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "fsbdd.auxiliary.analysis.fragment_map_analysis",
                "--probe",
            ],
            check=True,
            capture_output=True,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m fsbdd.auxiliary.analysis.fragment_map_analysis"
    )
