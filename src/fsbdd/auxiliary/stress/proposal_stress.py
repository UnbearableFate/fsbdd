...
def run_role(
    *,
    ...
    if min(fragment_count, payload_bytes, history_objects) <= 0:
        raise ValueError(
            "fragment_count, payload_bytes, and history_objects must be positive"
        )
    if s_max < 0:
    ...
    if rank == 0:
        ...
        if not entered.wait(timeout=60):
            raise TimeoutError(
                "sequence-2 publication task did not reach its delay point"
            )
    ...
    if any(item.payload_bytes != payload_bytes for item in discovered):
        raise AssertionError(
            "decoded proposal payload byte count differs from the workload"
        )
    fragment_results = []
...
def summarize(
    *,
    ...
    if (
        writer["conflict_new_payloads"] != 0
        or not writer["same_sequence_conflict_rejected"]
    ):
        raise AssertionError(
            "same-sequence conflict was not rejected before payload write"
        )
    if (
    ...
    for fragment in reader["fragment_results"]:
        ...
        if any(item["staleness"] != 0 for item in weights):
            raise AssertionError(
                "S1-05 formal workload did not run with fresh proposals"
            )
    ...
    summary = {
        ...
        "discovery": {
            ...
            "post_10000_object_read_calls": reader["post_history_discovery_read_calls"],
        ...
        "stale_ready_interfaces": {
            ...
            "bounded_base_retention_component": {
                ...
                "runtime_s_max": 0,
                "retained_base_entries": bounded_authority["retained_base_entries"],
                "maximum_base_entries": bounded_authority["maximum_base_entries"],
            },
        },
    }
...
def write_manifest(args: argparse.Namespace) -> None:
    ...
    if set(role_map["proposal_writer"] + role_map["proposal_discoverer"]) != set(hosts):
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m fsbdd.auxiliary.stress.proposal_stress"
    )
