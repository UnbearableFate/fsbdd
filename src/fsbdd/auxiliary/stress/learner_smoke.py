...
from fsbdd.diloco.learner.publication import build_fragment_parameter_groups
...
def profile_set_digest(profiles: Sequence[FrozenLearnerProfile]) -> str:
    if not profiles or len({profile.profile_id for profile in profiles}) != len(
        profiles
    ):
        raise LearnerSmokeError("profile set must be non-empty with unique identities")
    return canonical_digest(
        {
            profile.profile_id: profile.digest
...
def _batch_iterator(
    shard: PackedTokenShard, batch_size: int
...
def _offline_environment() -> dict[str, str]:
    ...
    if any(value != "1" for value in result.values()):
        raise LearnerSmokeError(
            f"real smoke requires offline-only environment: {result}"
        )
    return result
...
def _optimizer_state_dtypes(optimizer: Any) -> tuple[str, ...]:
    ...
    if dtypes != ["torch.float32"]:
        raise LearnerSmokeError(
            f"optimizer floating state must be fp32, observed {dtypes}"
        )
    return tuple(dtypes)
    ...
    *,
    ...
    runtime = LearnerRuntime(
        ...
        gradient_accumulation_steps=int(
            profile.training["gradient_accumulation_steps"]
        ),
    ...
    result = {
        ...
        "assets": {
            ...
            "model_file_hashes": {
                row["path"]: row["sha256"] for row in inventory["model_files"]
            },
            "tokenizer_file_hashes": {
                row["path"]: row["sha256"] for row in inventory["tokenizer_files"]
            },
            "source_file_hashes": {
                row["path"]: row["sha256"] for row in inventory["source_files"]
            },
            "materialized_manifest_sha256": _file_digest(
                materialized_root / "manifest.json"
            ),
            "materialized_marker_sha256": _file_digest(
                materialized_root / "complete.json"
            ),
        },
        "model": {
            **registry,
...
def run_tiny(
    output: Path, *, log_path: Path, device_name: str = "cuda"
) -> dict[str, Any]:
    import torch
    ...
    result = {
        ...
        "profile": {
            "profile_id": "s1-06-tiny-neox",
            "digest": canonical_digest({"seed": 1606}),
        },
        "host": {
            "hostname": socket.gethostname().split(".")[0],
            "device": str(device),
...
def summarize_runs(
    tiny: Path, gpt2: Path, pythia: Path, output: Path
) -> dict[str, Any]:
    inputs = {
    ...
    for name, row in inputs.items():
        ...
        if any(event.get("microbatches") != expected_microbatches for event in events):
            raise LearnerSmokeError(
                f"{name} gradient-accumulation boundaries disagree with the profile"
            )
        losses = [event.get("token_weighted_loss") for event in events]
        norms = [
            norm for event in events for norm in event.get("fragment_update_norms", [])
        ]
        if (
            not losses
            or not norms
            or any(
                ...
                for value in [*losses, *norms]
            )
        ):
            raise LearnerSmokeError(
                f"{name} loss or fragment norm is missing/nonfinite"
            )
        if (
            not runtime.get("parameters_changed")
            or runtime.get("parameter_update_norm", 0) <= 0
        ):
        ...
        if name in expected_real:
            ...
            if (
                ...
                or model.get("initial_parameter_sha256")
                == model.get("final_parameter_sha256")
            ):
            ...
            if (
                runtime.get("processed_input_tokens") != 20_480
                or runtime.get("loss_bearing_target_tokens") != 20_440
            ):
                raise LearnerSmokeError(f"{name} packed-512 token accounting mismatch")
        first = events[0]
        last = events[-1]
        interval = (
            runtime["training_end_monotonic_ns"]
            - runtime["training_start_monotonic_ns"]
        ) / 1_000_000_000
        ...
        if not math.isclose(
            ...
            raise LearnerSmokeError(
                f"{name} throughput is not its common-interval rate"
            )
        ...
        if any(value != 10 for value in last["fragment_local_steps"]):
            raise LearnerSmokeError(
                f"{name} per-fragment optimizer-step counters disagree"
            )
        if any(
            value != runtime["processed_input_tokens"]
            for value in last["fragment_processed_input_tokens"]
        ):
            raise LearnerSmokeError(
                f"{name} per-fragment input-token counters disagree"
            )
        profiles[name] = {
            ...
            "final_fragment_processed_input_tokens": last[
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m fsbdd.auxiliary.stress.learner_smoke"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "run-real":
        ...
        print(
            json.dumps(
                {"status": summary["status"], "profile": summary["profile"]},
                sort_keys=True,
            )
        )
        return 0
    ...
    if args.command == "manifest":
        ...
        print(
            json.dumps(
                {"status": "building", "output": str(args.output)}, sort_keys=True
            )
        )
        return 0
