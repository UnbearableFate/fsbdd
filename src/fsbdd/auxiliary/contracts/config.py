...
def load_config(path: Path, *, environ: dict[str, str] | None = None) -> ResolvedConfig:
    ...
    if conflicts:
        raise ConfigError(
            f"environment overrides are forbidden for frozen config: {conflicts}"
        )
    try:
    ...
    algorithm = _strict(
        top["algorithm"],
        "algorithm",
        {"S_max", "lambda_s", "Q", "Q_fresh", "grace_seconds"},
    )
    ...
    stop = _strict(
        top["stop"], "stop", {"target_global_cycles", "max_walltime_seconds"}
    )
    ...
    if (
        not isinstance(lambda_s, (int, float))
        or isinstance(lambda_s, bool)
        or lambda_s < 0
    ):
    ...
    offsets = tuple(
        _positive_int(value, "schedule.offset", allow_zero=True)
    ...
    return ResolvedConfig(
        ...
        model=AssetConfig(
            _nonempty(model["repository"], "model.repository"),
            _nonempty(model["revision"], "model.revision"),
        ),
        data=DataConfig(
            _nonempty(data["repository"], "data.repository"),
            _nonempty(data["revision"], "data.revision"),
            _nonempty(data["split"], "data.split"),
        ),
        fragment_map=FragmentMapConfig(identity, count),
        storage=StorageConfig(
            _nonempty(storage["backend"], "storage.backend"),
            Path(_nonempty(storage["root"], "storage.root")),
        ),
        stop=StopConfig(
            _positive_int(stop["target_global_cycles"], "stop.target_global_cycles"),
            _positive_int(stop["max_walltime_seconds"], "stop.max_walltime_seconds"),
        ),
