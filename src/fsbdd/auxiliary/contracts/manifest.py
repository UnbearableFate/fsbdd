...
def _mapping(
    value: Any, field: str, required: set[str], optional: set[str] | None = None
) -> dict[str, Any]:
    ...
    if missing or unknown:
        raise ManifestError(
            f"{field} shape mismatch: missing={sorted(missing)} unknown={sorted(unknown)}"
        )
    return value
...
def _validate_scheduler(scheduler: Any) -> dict[str, Any]:
    ...
    if not isinstance(scheduler["modules"], list) or any(
        not isinstance(item, str) or not item for item in scheduler["modules"]
    ):
...
def _validate_identities(identities: dict[str, Any]) -> None:
    ...
    code = _mapping(
        identities["code"], "code", {"repository", "branch", "commit", "dirty"}
    )
    ...
    execution = _mapping(
        ...
        "execution",
        {"identity", "initial_hostname", "workflow"},
        {"compute_hostname"},
    )
    ...
    *,
    ...
    manifest = {
        ...
        "run_identity": {
            ...
            **copy.deepcopy(identities["execution"]),
        },
        "identities": {
...
def validate_manifest(manifest: dict[str, Any], *, finalized: bool = True) -> None:
    ...
    identities["execution"] = {
        ...
        if key not in {"timestamp_utc", "time_source"}
    }
    _validate_identities(identities)
