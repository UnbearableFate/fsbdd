...
def validate_sweep_config(config: dict[str, Any]) -> None:
    ...
    if (
        execution.get("shard_count", 0) <= 0
        or execution.get("checkpoint_every_rows", 0) <= 0
    ):
...
def _simulation_config(
    spec: dict[str, Any], *, max_trace_events: int | None = None
) -> SimulationConfig:
...
def _row_integrity_digest(row: dict[str, Any]) -> str:
    normalized = {
        field: str(row[field]) for field in CSV_FIELDS if field != "row_digest"
    }
...
def _validate_retained_row(
    ...
    for field in expected_spec_fields:
        if row[field] != str(spec[field]):
            raise SweepInputError(
                f"retained row field differs from frozen spec: {field}"
            )
    numeric_fields = (
        "visibility_delay_over_h",
        "accepted_token_efficiency",
    ...
    if (
        ...
        or not math.isclose(
            intervals["mean"], float(row["update_interval_mean"]), abs_tol=1e-15
        )
...
def run_shard(
    ...
    for index, spec in enumerate(specs):
        ...
        if completed_since_checkpoint >= checkpoint_every:
            _write_csv(
                output_path, [existing[key] for key in sorted(existing)], CSV_FIELDS
            )
            if max_new_rows is None:
                completed_since_checkpoint = 0
    _write_csv(output_path, [existing[key] for key in sorted(existing)], CSV_FIELDS)
    expected = sum(
        1 for index in range(len(specs)) if index % shard_count == shard_index
    )
...
def _profile_aggregates(rows: Sequence[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for key in sorted(groups):
        ...
        record["visibility_delay_over_h"] = _number(
            members[0], "visibility_delay_over_h"
        )
...
def _recovery_records(
    ...
    for key in sorted(paired):
        ...
        record["gain_smax1_vs_smax0"] = (
            _number(arms[1], "accepted_token_efficiency") - fresh
        )
        record["gain_smax2_vs_smax0"] = (
            _number(arms[2], "accepted_token_efficiency") - fresh
        )
        output.append(record)
...
def _classification(gain_fraction: float, config: dict[str, Any]) -> str:
    thresholds = config["recommendation_policy"][
        "paper_classification_percentage_points"
    ]
    ...
    for q_fraction in policy["quorum_fraction"]:
        for grace in policy["grace_fraction_of_h"]:
            selected_rows = [
                ...
                and _number(row, "visibility_delay_seconds")
                in region["visibility_delay_seconds"]
            ]
            selected_recovery = [
                item
                ...
                and item["visibility_delay_seconds"]
                in region["visibility_delay_seconds"]
            ]
            rankings.append(
                {
                    ...
                    "mean_accepted_token_efficiency": statistics.fmean(
                        _number(row, "accepted_token_efficiency")
...
def _metric_summary(rows: Sequence[dict[str, str]]) -> dict[str, Any]:
    return {
        metric: _stats([_number(row, metric) for row in rows])
        for metric in METRIC_FIELDS
    }
...
def _build_summary(
    ...
    for axis in AXIS_ORDER:
        ...
        marginals[axis] = [
            {
                "value": value,
                "rows": len(axis_groups[value]),
                "metrics": _metric_summary(axis_groups[value]),
            }
    ...
    return {
        ...
        "recovery": {
            ...
            "paired_seed_level": _stats(
                [item["gain_smax1_vs_smax0"] for item in recovery]
            ),
...
def aggregate_shards(
    ...
    manifest = {
        ...
        "config_canonical_sha256": _sha256_bytes(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        _canonical_json({"rows": manifest["matrix"]["actual_rows"], "complete": True})
    )
    return 0
