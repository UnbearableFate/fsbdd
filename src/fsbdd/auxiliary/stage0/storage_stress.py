...
def _read_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise StorageHarnessError(
            f"cannot read JSONL evidence {path}: {error}"
        ) from error
    for line_number, line in enumerate(lines, start=1):
        ...
        if not isinstance(value, dict):
            raise StorageHarnessError(
                f"JSONL evidence {path}:{line_number} is not an object"
            )
    ...
    prefix = _canonical_json(
        {
            ...
            "sha256": hashlib.sha256(
                f"{run_id}:background:{sequence}".encode()
            ).hexdigest(),
        }
    ).encode("utf-8")
    if len(prefix) > size:
    ...
    *, config: Mapping[str, Any], run_id: str, test_root: Path, result_root: Path
    ...
    for sequence, profile, cache_state in _latency_schedule(config):
        ...
        if acknowledgement.get("reader_host") == record["writer_host"]:
            raise StorageHarnessError(
                "latency writer and reader were placed on the same host"
            )
        if invalid_ack_reads or acknowledgement["reader_invalid_record_reads"]:
            raise StorageHarnessError(
                "an invalid record was observed during the latency probe"
            )
        samples.append(
            {
                **record,
    ...
    if background_thread is None:
        raise StorageHarnessError(
            "loaded latency profile did not start its background load"
        )
    ...
    if background_state["error"]:
        raise StorageHarnessError(
            f"background writer failed: {background_state['error']}"
        )
    if background_state["replacements"] < int(
        visibility["background_minimum_replacements"]
    ):
    ...
    return {
        ...
        "cache_state_counts": dict(
            Counter(sample["cache_state"] for sample in samples)
        ),
...
def _run_latency_reader(
    *, config: Mapping[str, Any], run_id: str, test_root: Path
    ...
    for sequence, profile, cache_state in _latency_schedule(config):
        ...
        if (
            record["timing_method"] != TIMING_METHOD
            or record["timing_origin"] != TIMING_ORIGIN
        ):
            raise StorageHarnessError(
                "latency publication has an invalid timing schema"
            )
        ...
        if (
            observed_size != record["payload_bytes"]
            or observed_digest != record["payload_sha256"]
        ):
            raise StorageHarnessError(
                "latency payload is incomplete or has the wrong checksum"
            )
    ...
    if background_thread.is_alive():
        raise StorageHarnessError(
            "background reader did not observe its completion record"
        )
    if background_state["error"]:
        raise StorageHarnessError(
            f"background reader failed: {background_state['error']}"
        )
    if background_state["violations"]:
        raise StorageHarnessError(
            "background load reader observed an incomplete replacement"
        )
    if background_state["observations"] <= 0:
        raise StorageHarnessError(
            "background reader did not observe the loaded profile"
        )
    return {
...
def build_atomic_record(run_id: str, record_size: int, sequence: int) -> bytes:
    ...
    content = hashlib.sha256(
        f"{run_id}:atomic:{record_size}:{sequence}".encode()
    ).hexdigest()[:16]
    record = {
        "complete": True,
        "content": content,
    ...
    if len(encoded) > record_size:
        raise StorageHarnessError(
            f"record metadata does not fit in {record_size} bytes"
        )
...
def validate_atomic_record(content: bytes, *, run_id: str, record_size: int) -> int:
    ...
    if set(record) != required:
        raise StorageHarnessError(
            "atomic record schema is incomplete or contains extra fields"
        )
...
def _run_atomic_writer(
    *, config: Mapping[str, Any], run_id: str, test_root: Path, result_root: Path
    ...
    for record_size in atomicity["record_sizes_bytes"]:
        ...
        replace_visibility_bytes(
            record_path, build_atomic_record(run_id, record_size, -1)
        )
        ...
        for record_size_value in atomicity["record_sizes_bytes"]:
                    ...
                    with checkpoint_path.open("x", encoding="utf-8") as checkpoint:
                        ...
                        while time.monotonic() <= deadline:
                            ...
                            try:
                                ...
                                state["final_seen"] = (
                                    state["final_seen"] or sequence == count - 1
                                )
                            ...
                            if (
                                ...
                                and state["final_seen"]
                                and state["observations"] >= minimum
                            ):
                                break
                            time.sleep(poll)
                        else:
            ...
            if any(state["error"] for state in states):
                raise StorageHarnessError(
                    ...
                    + "; ".join(
                        str(state["error"]) for state in states if state["error"]
                    )
                )
            atomic_write_json(
                test_root / "atomic" / f"reader-done-{record_size}.json",
...
def _run_kill_probe_writer(
    *, run_id: str, test_root: Path, timeout: float
) -> dict[str, Any]:
    ...
    for position in ("before_visibility", "after_visibility"):
        ...
        if (
            not os.WIFSIGNALED(wait_status)
    ...
    for position in ("before_visibility", "after_visibility"):
        ...
        if position == "before_visibility":
            if visibility_path.exists():
                raise StorageHarnessError(
                    "killed pre-publication writer exposed visibility"
                )
        else:
            ...
            if payload_path.stat().st_size != record["payload_bytes"]:
                raise StorageHarnessError(
                    "post-publication kill payload length mismatch"
                )
            if file_sha256(payload_path) != record["payload_sha256"]:
                raise StorageHarnessError(
                    "post-publication kill payload checksum mismatch"
                )
        atomic_write_json(
            root / "reader-verified.json",
            {
...
def _run_unsafe_reader(
    *, config: Mapping[str, Any], run_id: str, test_root: Path, result_root: Path
    ...
    states = [
        {"reader_id": index, "observations": 0, "violations": 0}
    ...
    if violation_log.count <= 0:
        raise StorageHarnessError(
            "unsafe direct overwrite produced no detected partial reads"
        )
    atomic_write_json(
        test_root / "unsafe" / "reader-done.json",
        {"schema_version": 1, "run_id": run_id, "sequence": iterations - 1},
...
def run_stress_role(
    *,
    ...
    try:
        ...
        raise StorageHarnessError(
            "storage stress path must be below the frozen RUN_ROOT"
        ) from error
...
def validate_latency_samples(
    ...
    visibility = config["visibility"]
    expected_total = int(visibility["samples_per_profile"]) * len(
        visibility["profiles"]
    )
    if len(samples) != expected_total:
        raise StorageHarnessError(
            f"latency sample count {len(samples)} != {expected_total}"
        )
    combinations = Counter(
        (sample.get("profile"), sample.get("cache_state")) for sample in samples
    )
    ...
    if dict(combinations) != expected_combinations:
        raise StorageHarnessError(
            "latency evidence is missing a profile or cache-state sample"
        )
    ...
    for sample in samples:
        if sample.get("timing_method") != TIMING_METHOD:
            raise StorageHarnessError(
                "latency sample used a non-conservative timing method"
            )
        if sample.get("timing_origin") != TIMING_ORIGIN:
            raise StorageHarnessError(
                "latency sample began before visibility publication completed"
            )
        ...
        if sample.get("invalid_record_reads") != 0:
            raise StorageHarnessError(
                "latency sample includes an incomplete record observation"
            )
    profiles: dict[str, Any] = {}
    threshold = float(config["thresholds"]["visibility_p99_seconds"])
    for profile in visibility["profiles"]:
        values = [
            int(sample["writer_ack_roundtrip_ns"])
            for sample in samples
        ...
        if statistics["p99_seconds"] > threshold:
            raise StorageHarnessError(
                f"{profile} visibility p99 exceeds {threshold} seconds"
            )
...
def validate_atomicity_results(
    ...
    *,
    ...
    writer_sizes = {
        int(item["record_size"]): item for item in writer.get("record_sizes", [])
    }
    reader_sizes = {
        int(item["record_size"]): item for item in reader.get("record_sizes", [])
    }
    ...
    for record_size in expected_sizes:
        ...
        if writer_result.get("publication_transcript_records") != expected_count:
            raise StorageHarnessError(
                "atomic writer transcript has the wrong record count"
            )
        ...
        if Path(transcript_name).name != transcript_name:
            raise StorageHarnessError(
                "atomic writer transcript path is not a safe basename"
            )
        if result_root is not None:
            ...
            for sequence, publication in enumerate(transcript):
                ...
                if publication_run_id is None or (
                    ...
                    raise StorageHarnessError(
                        "atomic publication run identity is invalid"
                    )
                ...
                if publication.get("sequence") != sequence:
                    raise StorageHarnessError(
                        "atomic publication transcript sequence is incomplete"
                    )
                if publication.get("timing_origin") != TIMING_ORIGIN:
                    raise StorageHarnessError(
                        "atomic publication timing origin is invalid"
                    )
                publication_ns = publication.get("publication_complete_monotonic_ns")
                if isinstance(publication_ns, bool) or not isinstance(
                    publication_ns, int
                ):
                    raise StorageHarnessError("atomic publication timestamp is invalid")
                if (
                    last_publication_ns is not None
                    and publication_ns < last_publication_ns
                ):
        ...
        if not isinstance(readers, list) or len(readers) != int(
            atomicity["concurrent_readers"]
        ):
            raise StorageHarnessError(
                "atomicity evidence has the wrong concurrent reader count"
            )
        for state in readers:
            if state.get("violations") != 0:
                raise StorageHarnessError(
                    "an atomic reader observed an incomplete record"
                )
            if int(state.get("observations", 0)) < int(
                atomicity["minimum_observations_per_reader"]
            ):
                raise StorageHarnessError("an atomic reader made too few observations")
            if state.get("final_seen") is not True:
                raise StorageHarnessError(
                    "an atomic reader did not observe the final replacement"
                )
...
def summarize_stress(
    *, config: Mapping[str, Any], run_id: str, result_root: Path, mode: str
    ...
    roles = [
        read_json_record(result_root / f"stress-rank-{rank}.json") for rank in (0, 1)
    ]
    ...
    if mode == "unsafe":
        ...
        if observed <= 0 or len(raw_violations) != observed:
            raise StorageHarnessError(
                "unsafe detector summary does not match its raw scanner log"
            )
        summary.update(
            {
        ...
        if writer["latency"].get("background_replacements", 0) < int(
            ...
            raise StorageHarnessError(
                "loaded profile background replacement count is too low"
            )
        loaded_start = int(
            writer["latency"].get("loaded_profile_start_replacements", 0)
        )
        ...
        if loaded_start < int(config["visibility"]["background_minimum_replacements"]):
            raise StorageHarnessError(
                "loaded profile began before background-load warmup"
            )
        if loaded_end <= loaded_start:
            raise StorageHarnessError(
                "background load was not active throughout loaded sampling"
            )
        if reader["latency"].get("background_violations") != 0:
            raise StorageHarnessError(
                "loaded profile observed a partial background record"
            )
        ...
        for role in roles:
            ...
            if {probe.get("position") for probe in probes} != expected_positions:
                raise StorageHarnessError(
                    "writer-kill evidence is missing a publication boundary"
                )
...
def finalize_stress_manifest(
    *, manifest_path: Path, summary_path: Path, yaml_output_path: Path
    ...
    summary = read_json_record(
        summary_path, required_fields=("run_id", "mode", "outcome", "roles")
    )
    if summary["run_id"] != manifest.get("run_id"):
        raise StorageHarnessError(
            "stress summary and environment manifest run_id differ"
        )
    roles = summary["roles"]
