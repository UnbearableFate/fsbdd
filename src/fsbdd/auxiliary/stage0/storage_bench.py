...
def deterministic_payload(
    size: int, *, seed: str = "fsbdd-stage0-storage-smoke"
) -> bytes:
...
def validate_storage_config(config: Mapping[str, Any]) -> None:
    ...
    if not root.is_absolute() or not str(root).startswith("/work/"):
        raise StorageHarnessError(
            "target_shared_run_root must be an absolute /work path"
        )
    if miyabi.get("expected_filesystem_type") != "lustre":
        raise StorageHarnessError(
            "Stage 0 target filesystem type must be frozen as lustre"
        )
    if miyabi.get("compute_nodes") != 2 or miyabi.get("ranks_per_node") != 1:
        raise StorageHarnessError(
            "the smoke topology must be two nodes and one role per node"
        )
    ...
    if thresholds.get("metadata_required_ops_per_second") != 5120.0:
        raise StorageHarnessError(
            "metadata capacity threshold must be 10x steady demand"
        )
    ...
    if not isinstance(visibility, Mapping) or not isinstance(atomicity, Mapping):
        raise StorageHarnessError(
            "visibility and atomicity benchmark mappings are required"
        )
    if visibility.get("profiles") != ["empty", "loaded"]:
        raise StorageHarnessError(
            "visibility profiles must be frozen as empty and loaded"
        )
    if visibility.get("cache_states") != ["cold", "warm"]:
        raise StorageHarnessError(
            "visibility cache states must be frozen as cold and warm"
        )
    if visibility.get("samples_per_profile") != 1000:
        raise StorageHarnessError(
            "visibility requires exactly 1000 samples per profile"
        )
    ...
    if visibility.get("timing_method") != "writer_ack_roundtrip_upper_bound":
        raise StorageHarnessError(
            "visibility timing must use writer-side acknowledgement RTT"
        )
    if visibility.get("timing_origin") != "visibility_record_replace_complete":
        raise StorageHarnessError(
            "visibility timing must begin after visibility replacement"
        )
    ...
    if atomicity.get("replacements_per_record_size") != 100000:
        raise StorageHarnessError(
            "atomicity requires 100000 replacements per record size"
        )
    ...
    if atomicity.get("writer_kill_positions") != [
        ...
        raise StorageHarnessError(
            "both writer-kill publication boundaries are required"
        )
    ...
    if not isinstance(metadata, Mapping) or not isinstance(bandwidth, Mapping):
        raise StorageHarnessError(
            "metadata and bandwidth benchmark mappings are required"
        )
    ...
    if metadata.get("history_sizes_for_negative_control") != [0, 1000, 10000]:
        raise StorageHarnessError(
            "metadata history controls must be 0, 1000, and 10000"
        )
    for field in (
        "repeats",
        "sustained_seconds",
    ...
    if bandwidth.get("payload_sizes_mb") != [50, 100, 250, 500]:
        raise StorageHarnessError(
            "bandwidth payload sizes must cover 50 through 500 MB"
        )
    if bandwidth.get("streams") != [1, 16]:
        raise StorageHarnessError(
            "bandwidth streams must be single and M=16 concurrent"
        )
    ...
    if bandwidth.get("cross_host_rank_offset") != 8:
        raise StorageHarnessError(
            "capacity readers must use the opposite-host rank offset"
        )
...
def _role_identity() -> tuple[int, int]:
    try:
        ...
        raise StorageHarnessError(
            "smoke-role requires an Open MPI rank context"
        ) from error
    ...
    *,
    ...
    if path_mode == "shared":
        try:
            ...
            raise StorageHarnessError(
                "shared smoke path must be below target RUN_ROOT"
            ) from error
    ...
    try:
        if rank == 0:
            ...
            if acknowledgement["run_id"] != run_id:
                ...
                raise StorageHarnessError(
                    "reader acknowledgement has a different run_id"
                )
            if acknowledgement["payload_bytes"] != len(payload):
                result["failure_kind"] = "acknowledgement_mismatch"
                raise StorageHarnessError(
                    "reader acknowledgement has a different payload size"
                )
            if acknowledgement["payload_sha256"] != expected_digest:
                result["failure_kind"] = "acknowledgement_mismatch"
                raise StorageHarnessError(
                    "reader acknowledgement has a different digest"
                )
            ...
            if (
                ...
                raise StorageHarnessError(
                    "visible payload does not match its publication record"
                )
            acknowledgement = {
                "schema_version": 1,
                "run_id": run_id,
...
def validate_environment_manifest(
    ...
    *,
    ...
    if (
        ...
        raise StorageHarnessError(
            "environment manifest requires two distinct compute hosts"
        )
    ...
    if require_role_mapping:
        ...
        if not isinstance(role_mapping, list) or len(role_mapping) != 2:
            raise StorageHarnessError(
                "actual role mapping must contain exactly two roles"
            )
        ...
        if {role.get("role") for role in role_mapping} != {"writer", "reader"}:
            raise StorageHarnessError(
                "actual role mapping must contain writer and reader"
            )
        if {role.get("hostname") for role in role_mapping} != set(hosts):
            raise StorageHarnessError(
                "actual role mapping hosts differ from the allocation"
            )
        if any(
            role.get("pbs_job_id") != manifest["pbs_job_id"] for role in role_mapping
        ):
...
def build_environment_manifest(
    *,
    ...
    compute_hosts = sorted(
        set(
            line.strip()
            for line in pbs_nodefile_path.read_text(encoding="utf-8").splitlines()
        )
    )
    manifest = {
        "schema_version": 1,
...
def finalize_environment_manifest(
    *,
    ...
    if summary["run_id"] != manifest.get("run_id"):
        raise StorageHarnessError(
            "smoke summary and environment manifest run_id differ"
        )
    roles = summary["roles"]
    ...
    *,
    ...
    if expected == "pass":
        ...
        if len({result.get("payload_sha256") for result in role_results}) != 1:
            raise StorageHarnessError(
                "shared-path roles reported different payload digests"
            )
        if len({result.get("payload_bytes") for result in role_results}) != 1:
            raise StorageHarnessError(
                "shared-path roles reported different payload sizes"
            )
        if role_results[0].get("reader_host") != role_results[1].get("hostname"):
            raise StorageHarnessError(
                "writer acknowledgement did not identify the reader host"
            )
        ...
        if path_mode != "local-negative" or statuses != ["failed", "failed"]:
            raise StorageHarnessError(
                "node-local negative control did not fail on both roles"
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    role.add_argument(
        "--path-mode", choices=("shared", "local-negative"), required=True
    )
