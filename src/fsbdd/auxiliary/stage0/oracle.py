...
def pseudo_gradient(base: Sequence[float], local: Sequence[float]) -> list[float]:
    ...
    return [
        _f32(left - right)
        for left, right in zip(base_values, local_values, strict=True)
    ]
...
def validated_pseudo_gradient(
    *,
    ...
    if any(
        ...
        for value in (base_version, current_version, s_max)
    ):
...
def inverse_staleness_weights(
    ...
    if len(tokens) == 0 or len(tokens) != len(staleness):
        raise OracleInputError(
            "tokens and staleness must be non-empty and shape-matched"
        )
    ...
    for token_value, stale_value in zip(tokens, staleness, strict=True):
        ...
        if (
            isinstance(stale_value, bool)
            or not isinstance(stale_value, int)
            or stale_value < 0
        ):
...
class CandidatePolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            ...
            raise OracleInputError(
                "candidate-policy count/version fields must be integers"
            )
...
def select_proposals(
    ...
    for proposal in proposals:
        if proposal.proposal_id in seen_ids:
            raise OracleInputError(
                f"duplicate proposal identity: {proposal.proposal_id}"
            )
...
def weighted_direct_merge(
    *,
    ...
    if not bases or len(bases) != len(locals_) or len(bases) != len(weights):
        raise OracleInputError(
            "bases, locals, and weights must have equal non-zero length"
        )
