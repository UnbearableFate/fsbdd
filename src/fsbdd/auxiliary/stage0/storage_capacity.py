...
def _rank_identity() -> tuple[int, int]:
    try:
        ...
        raise StorageHarnessError(
            "capacity role requires an Open MPI rank identity"
        ) from error
    return rank, world
    ...
    *,
    ...
    if profile == "stat":
        ...
        if layout == "flat":
            ...
            expected = fragments
        references = sum(
            1 for entry in os.scandir(directory) if entry.name.endswith(".json")
        )
    ...
    if state == "first_touch":
        if profile == "readdir":
            ...
            for fragment in range(fragments):
                ...
                path = _slot_path(
                    root, layout=layout, learner=learner, fragment=fragment
                )
                if profile == "stat":
                    path.stat()
                else:
                    ...
                    if (
                        ...
                        or record.get("fragment") != fragment
                    ):
                ...
                for fragment in range(fragments):
                    ...
                    if profile == "stat":
                        ...
                        if (
                            ...
                            raise StorageHarnessError(
                                "fixed-slot read identity mismatch"
                            )
                    sample.add(time.monotonic_ns() - operation_started)
                    references += 1
                    syscalls += 1
...
def _run_metadata_rank(
    ...
    for learners, fragments, layout, repeat, profile in metadata_matrix(config):
        active = rank < learners and (
            profile != "readdir" or layout != "flat" or rank == 0
        )
        root = _dataset_root(
            test_root,
            learners=learners,
                ...
                coordinator_rounds.append(
                    {
                        ...
                        "ready_ranks": [
                            int(record["rank"]) for record in ready_records
                        ],
...
def _history_controls(
    config: Mapping[str, Any], test_root: Path, run_id: str
) -> list[dict[str, Any]]:
    metadata = config["metadata"]
...
def _metadata_phase_id(
    *, learners: int, fragments: int, layout: str, repeat: int, profile: str, state: str
    ...
    return (
        f"metadata-m{learners}-f{fragments}-{layout}-repeat-{repeat}-{profile}-{state}"
    )
    ...
    for size_mb, streams, repeat in bandwidth_matrix(config):
        ...
        if rank in reader_ranks:
            source_stream = (
                0
                if streams == 1
                else (rank + int(bandwidth["cross_host_rank_offset"])) % streams
            )
...
def run_capacity_role(
    *, config: Mapping[str, Any], run_id: str, test_root: Path, result_root: Path
    ...
    try:
        ...
        raise StorageHarnessError(
            "capacity path must be below the frozen RUN_ROOT"
        ) from error
    ...
    _wait_json(
        ready, phase="ready", timeout=float(config["smoke"]["timeout_seconds"]) * 10
    )
    ...
    if rank == 0:
        ...
        state = {
            **setup,
            ...
            "payload_files_after": _count_matching(
                test_root / "payloads", lambda _path: True
            ),
            "temporary_files_after": _count_matching(
                test_root, lambda path: ".tmp-" in path.name
            ),
            "control_files_after": _count_matching(
                test_root / "control", lambda _path: True
            ),
...
def _metadata_summary(
    roles: Sequence[Mapping[str, Any]], config: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    for key in sorted(groups, key=lambda value: tuple(str(item) for item in value)):
        ...
        if (
            ...
            raise StorageHarnessError(
                "metadata coordinator interval provenance is invalid"
            )
        if any(
            ...
            raise StorageHarnessError(
                "metadata result lacks coordinator interval provenance"
            )
        ...
        latency = [
            int(value) for record in records for value in record["latency_samples_ns"]
        ]
        if (
            not latency
            or references <= 0
            ...
            raise StorageHarnessError(
                "metadata result contains no measurable operations"
            )
        summaries.append(
            {
                "learners": learners,
    ...
    return {
        ...
        "threshold": {
            ...
            "polling_interval_seconds": float(
                config["metadata"]["polling_interval_seconds"]
            ),
...
def _history_summary(
    ...
    for control in controls:
        if control["history_scan_counts"] != [control["history_objects"]] * int(
            ...
            raise StorageHarnessError(
                "history scan did not observe the expected object count"
            )
        if control["bounded_fixed_references"] != 16 * 32:
            raise StorageHarnessError(
                "bounded discovery did not remain fixed at M by F"
            )
        summarized.append(
            {
                **control,
    ...
    if not (medians[2] > medians[1] > medians[0]):
        raise StorageHarnessError(
            "history-scanning negative cost did not grow with history"
        )
    return {
...
def _bandwidth_summary(
    ...
    for size_mb, streams, repeat in bandwidth_matrix(config):
        ...
        writes = [
            ...
            if item["phase"] == phase and item["operation"] == "write"
        ]
        reads = [
            item
            for item in operations
            if item["phase"] == phase and item["operation"] == "read"
        ]
        ...
        for read in reads:
            ...
            if read["sha256"] != writer_digests.get(source):
                raise StorageHarnessError(
                    "cross-host sequential read checksum mismatch"
                )
            ...
            summaries.append(
                {
                    ...
                    "aggregate_mib_per_second": total_bytes
                    / (1024 * 1024)
                    / round_seconds,
    ...
    expected = (
        len(config["bandwidth"]["payload_sizes_mb"])
        * len(config["bandwidth"]["streams"])
        * int(config["bandwidth"]["repeats"])
        * 2
    )
    ...
    concurrent_writes = [
        ...
        if item["streams"] == 16 and item["operation"] == "write"
    ]
...
def summarize_capacity(
    *, config: Mapping[str, Any], run_id: str, result_root: Path
    ...
    world = int(config["bandwidth"]["launcher_world_size"])
    roles = [
        read_json_record(result_root / f"capacity-rank-{rank}.json")
        for rank in range(world)
    ]
    if {role.get("rank") for role in roles} != set(range(world)):
        raise StorageHarnessError("capacity role set is incomplete")
    if any(
        role.get("run_id") != run_id or role.get("status") != "passed" for role in roles
    ):
    ...
    if len(hosts) != 2 or sorted(hosts.values()) != [
        expected_per_host,
        expected_per_host,
    ]:
        raise StorageHarnessError(
            "capacity benchmark did not use eight ranks on each of two hosts"
        )
    first_host = {
        str(role["hostname"]) for role in roles if int(role["rank"]) < expected_per_host
    }
    second_host = {
        str(role["hostname"])
        for role in roles
        if int(role["rank"]) >= expected_per_host
    }
    if len(first_host) != 1 or len(second_host) != 1 or first_host == second_host:
        raise StorageHarnessError(
            "rank offset does not identify the opposite compute host"
        )
    ...
    if state["payload_files_after"] != 0 or state["temporary_files_after"] != 0:
        raise StorageHarnessError(
            "capacity benchmark left payload or temporary objects"
        )
...
def finalize_manifest(
    *, manifest_path: Path, summary_path: Path, output: Path
) -> dict[str, Any]:
...
def cleanup_capacity(*, config: Mapping[str, Any], test_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise StorageHarnessError(
            "cleanup path must be below the frozen RUN_ROOT"
        ) from error
