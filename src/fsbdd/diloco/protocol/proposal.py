...
def _f32(value: object) -> float:
    ...
    try:
        ...
        raise ProposalError(
            f"value is not representable as float32: {value!r}"
        ) from error
...
def encode_proposal(proposal: Proposal) -> bytes:
    ...
    header = canonical_bytes(
        {**semantic, "content_identity": proposal.content_identity}
    )
...
class ProposalStore:
    ...
    def _in_flight_lock(self, learner_id: str, fragment_index: int) -> threading.Lock:
        return self._in_flight[self.learner_ids.index(learner_id)][fragment_index]
    ...
    def publish(
        ...
        *,
        ...
        in_flight = self._in_flight_lock(proposal.learner_id, proposal.descriptor.index)
...
class EligibilityPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        max_contributors = _require_positive(self.max_contributors, "max_contributors")
        ...
        if any(
            ...
            raise ProposalError(
                "retained_bases must contain RetainedBaseIdentity values"
            )
...
def select_candidates(
    ...
    selected = tuple(ordered_distinct[: policy.max_contributors]) if ready else tuple()
...
def commit_consumption(
    ...
    for proposal in selected:
        ...
        if proposal.descriptor != frontiers.descriptor:
            raise ProposalError(
                "selected proposal and frontier fragment descriptors differ"
            )
