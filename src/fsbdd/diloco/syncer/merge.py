    ...
    def __post_init__(self) -> None:
        ...
        for contribution in self.contributions:
            ...
            if contribution.payload_bytes != len(current):
                raise MergeError(
                    "contribution payload shape differs from current fragment"
                )
            if (
                self.current_version - contribution.base_version
                != contribution.staleness
            ):
                raise MergeError(
                    "contribution staleness differs from current minus base"
                )
        ...
        if self.outer_state.momentum_buffer is not None and len(
            self.outer_state.momentum_buffer
        ) != len(current):
...
class ImmutableFileBaseSource:
    ...
    def open_base(self, contribution: ContributionFact) -> Iterator[ResolvedBase]:
        try:
            ...
            raise MergeError(
                "declared base is absent from the retained base source"
            ) from error
...
class MergePolicy(ABC):
    ...
    @abstractmethod
    def accumulate(
        self, accumulator: Any, base: Any, local: Any, weight: float
    ) -> None:
        raise NotImplementedError
...
def build_update_facts(
    ...
    return {
        ...
        "ordered_proposal_identities": [
            item.proposal_id for item in request.contributions
        ],
...
def execute_streaming_fragment_update(
    ...
    *,
    ...
    if not isinstance(policy, MergePolicy):
        raise MergeError("merge_policy must implement MergePolicy")
    if (
        outer_policy.f32_momentum == 0
        and request.outer_state.momentum_buffer is not None
    ):
    ...
    def observe_rss() -> None:
        ...
        if measure_process_rss:
            ...
            maximum_rss = (
                current_rss if maximum_rss is None else max(maximum_rss, current_rss)
            )
        ...
        with contribution_source.open_payload(contribution) as local_payload:
            ...
            try:
                ...
                if len(local_payload) != fragment_bytes:
                    raise MergeError(
                        "local contribution shape differs from current fragment"
                    )
                if (
                ...
                try:
                    if contribution.base_version == request.current_version:
                        if (
                            contribution.base_content_identity
                            != request.current_content_identity
                        ):
                            raise MergeError(
                                "current-base contribution identity mismatch"
                            )
                        base = current
                        policy.accumulate(
                        ...
                        if base_source is None:
                            raise MergeError(
                                "noncurrent contribution requires a retained base source"
                            )
                        with base_source.open_base(contribution) as resolved:
                            ...
                            try:
                                if (
                                    ...
                                    raise MergeError(
                                        "resolved base identity differs from proposal"
                                    )
                                if len(resolved.parameters) != fragment_bytes:
                                    raise MergeError(
                                        "resolved base shape differs from current fragment"
                                    )
    ...
    try:
        for contribution in request.contributions:
            with contribution_source.open_payload(contribution) as local_payload:
                ...
                if contribution.base_version != request.current_version:
                    raise MergeError(
                        "NumPy Profile A merge requires a current-base contribution"
                    )
                if (
                    contribution.base_content_identity
                    != request.current_content_identity
