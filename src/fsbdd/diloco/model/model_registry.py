...
def _builtin_mapping(model: Any) -> ExplicitMapping:
    ...
    raise RegistryError(
        f"unsupported or ambiguous model_type {model_type!r}; provide explicit mapping"
    )
...
def build_logical_layer_registry(
    ...
    *,
    ...
    if (
        not isinstance(sync_dtype_bytes, int)
        or isinstance(sync_dtype_bytes, bool)
        or sync_dtype_bytes <= 0
    ):
    ...
    unclassified_buffers = sorted(
        name
        for names in buffer_aliases.values()
        for name in names
        if name not in allowed_buffers
    )
        ...
        if candidates:
            if 0 in candidates:
                ...
                if candidates - {0, len(logical_paths) - 1}:
                    raise RegistryError(
                        f"parameter shared across unsupported logical layers: {names}"
                    )
                owner_index[key] = 0
    ...
    for assignment in mapping.misc:
        misc_keys = [
            key
            for key, names in aliases.items()
            if any(_under(name, assignment.module_path) for name in names)
        ]
        for key in misc_keys:
            if key in owner_index:
                raise RegistryError(
                    f"misc module overlaps an existing logical owner: {aliases[key]}"
                )
        left_bytes = layer_bytes[assignment.left_layer]
        right_bytes = layer_bytes[assignment.right_layer]
        selected = (
            assignment.left_layer
            if left_bytes <= right_bytes
            else assignment.right_layer
        )
        for key in misc_keys:
    ...
    if unowned:
        raise RegistryError(
            f"unowned trainable parameters require explicit mapping: {unowned}"
        )

    layer_names = (
        "embedding",
        *(f"block-{index}" for index in range(len(mapping.block_paths))),
        "lm_head",
    )
    ...
    validate_parameter_ownership(
        [(record.owner_layer, record.identity) for record in records]
    )
    ...
    layers = tuple(
        LogicalLayer(
            ...
            parameter_count=sum(
                ...
                if record.owner_layer == layer_names[index]
            ),
            sync_bytes=sum(
                record.sync_bytes
                for record in records
...
def write_registry_report(registry: LogicalLayerRegistry, destination: Path) -> str:
    ...
    destination.write_text(
        json.dumps(registry.to_dict(), sort_keys=True, indent=2) + "\n",
        encoding="utf-8",
    )
