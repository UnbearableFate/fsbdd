...
class FragmentMap:
    ...
    def _body(self) -> dict[str, Any]:
        return {
            ...
            "misc_assignments": [
                assignment.to_dict() for assignment in self.misc_assignments
            ],
...
def _validate_fragment_count(
    fragment_count: int, layer_count: int, production: bool
) -> None:
...
def partition_layer_bytes(
    ...
    if not weights or any(
        ...
        raise FragmentMapError(
            "layer synchronization bytes must be nonnegative integers"
        )
    if not isinstance(fragment_count, int) or isinstance(fragment_count, bool):
        raise FragmentMapError("fragment_count must be an integer")
    ...
    try:
        ...
        raise FragmentMapError(
            "no partition satisfies the computed minimax bound"
        ) from error
...
def _validate_registry(registry: LogicalLayerRegistry) -> None:
    ...
    flattened = [
        identity for layer in layers for identity in layer.parameter_identities
    ]
    if len(flattened) != len(set(flattened)) or set(flattened) != set(
        parameter_records
    ):
        raise FragmentMapError(
            "logical layers must cover every parameter identity exactly once"
        )
    if (
    ...
    for record in registry.parameters:
        if (
            record.numel < 0
            or record.sync_bytes != record.numel * registry.sync_dtype_bytes
        ):
            raise FragmentMapError(
                f"registry parameter synchronization bytes mismatch: {record.owner_name}"
            )
    for layer in layers:
        records = [
            parameter_records[identity] for identity in layer.parameter_identities
        ]
        if layer.parameter_count != sum(record.numel for record in records):
            raise FragmentMapError(
                f"logical-layer parameter count mismatch: {layer.name}"
            )
        if layer.sync_bytes != sum(record.sync_bytes for record in records):
            raise FragmentMapError(
                f"logical-layer synchronization bytes mismatch: {layer.name}"
            )
...
def build_fragment_map(
    ...
    *,
        ...
        fragments.append(
            Fragment(
                ...
                parameter_identities=tuple(
                    ...
                    for identity in layer.parameter_identities
                ),
    ...
    body = {
        ...
        "misc_assignments": [
            assignment.to_dict() for assignment in _misc_summaries(registry)
        ],
...
def validate_fragment_map(
    ...
    if fragment_map.registry_digest != registry.digest:
        raise FragmentMapError(
            "fragment map references a different logical-layer registry"
        )
    if fragment_map.sync_dtype_bytes != registry.sync_dtype_bytes:
        raise FragmentMapError(
            "fragment map sync_dtype_bytes differs from its registry"
        )
    ...
    _validate_fragment_count(
        fragment_map.fragment_count,
        fragment_map.layer_count,
        fragment_map.production_mode,
    )
    expected_start = 0
    ...
    for index, fragment in enumerate(fragment_map.fragments):
        ...
        selected = fragment_map.layers[
            fragment.start_layer : fragment.end_layer_exclusive
        ]
        ...
        if fragment.parameter_identities != expected_parameters:
            raise FragmentMapError(
                "fragment parameter identities do not match its complete layers"
            )
    ...
    if len(parameter_identities) != len(set(parameter_identities)) or set(
        parameter_identities
    ) != set(registry_identities):
        raise FragmentMapError(
            "every registry parameter must belong to exactly one fragment"
        )
    fragment_bytes = tuple(fragment.sync_bytes for fragment in fragment_map.fragments)
    expected_objective = partition_layer_bytes(
        tuple(layer.sync_bytes for layer in fragment_map.layers),
        fragment_map.fragment_count,
    )
    actual_cuts = tuple(
        fragment.end_layer_exclusive for fragment in fragment_map.fragments[:-1]
    )
    actual_objective = FragmentObjective(
        maximum_sync_bytes=max(fragment_bytes),
        total_deviation_scaled=sum(
            abs(fragment_map.fragment_count * size - sum(fragment_bytes))
            for size in fragment_bytes
        ),
        cut_indices=actual_cuts,
    )
    if fragment_map.objective != actual_objective:
        raise FragmentMapError(
            "fragment map objective does not describe its actual cuts and bytes"
        )
    if actual_objective != expected_objective:
        raise FragmentMapError(
            "fragment map actual cuts do not satisfy the frozen optimal objective"
        )
    ...
    expected_balance = BalanceSummary(
        ...
        maximum_to_average_ratio=maximum_bytes
        * fragment_map.fragment_count
        / total_bytes,
        average_exact_numerator=total_bytes,
...
def write_fragment_map_report(fragment_map: FragmentMap, destination: Path) -> str:
    if destination.exists():
        raise FragmentMapError(
            f"refusing to overwrite fragment map report: {destination}"
        )
    destination.parent.mkdir(parents=True, exist_ok=True)
    destination.write_text(
        json.dumps(fragment_map.to_dict(), sort_keys=True, indent=2) + "\n",
        encoding="utf-8",
    )
