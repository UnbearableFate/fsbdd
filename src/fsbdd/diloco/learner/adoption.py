...
from fsbdd.diloco.protocol.global_state import FragmentGlobalState, GlobalStateStore
...
def optimizer_fragment_state_entry_count(
    ...
    return sum(
        bool(optimizer.state.get(parameter, {})) for parameter in fragment_parameters
    )
    ...
    def __init__(
        ...
        *,
        ...
        if len(contents) != len(versions):
            raise AdoptionError(
                "adopted content identities must match adopted versions"
            )
    ...
    def summary(self) -> dict[str, Any]:
        with self._condition:
            ...
            return {
                ...
                "adopted_content_identities": list(self._adopted_content_identities),
...
class FragmentAdoptionCoordinator:
    ...
    def __init__(
        ...
        *,
        ...
        for state, descriptor, fragment, group in zip(
            ...
            if (
                sum(int(parameter.numel()) for parameter in group)
                != descriptor.shape[0]
            ):
    ...
    def _decode_sources(self, observation: PendingFragmentAdoption) -> tuple[Any, ...]:
        import torch
        ...
        if array.size != descriptor.shape[0] or not bool(np.isfinite(array).all()):
            raise AdoptionError(
                "global fragment parameters contain NaN Inf or wrong shape"
            )
        sources = []
        offset = 0
    ...
    def _audit_hashes(self) -> tuple[list[str], list[str], list[int]]:
        if not self.identity_audit or self.optimizer is None:
            return [], [], []
        parameters = [
            fragment_payload_sha256(group) for group in self.fragment_parameters
        ]
        moments = [
    ...
    def _apply_one(
        ...
        completed = self.clock_ns()
        adoption_unix_ns = _nonnegative_int(self.wall_clock_ns(), "adoption wall clock")
        ...
        if self.identity_audit:
            ...
            if (
                ...
                raise AdoptionError(
                    "adopted target differs from verified global payload"
                )
            if (
