...
from .learner import ConstantStepScheduler, LearnerProgress, LearnerRng, LearnerRuntime
...
def _writer(
    *,
    ...
    if identities.run_identity != run_id or identities.config_identity != config_identity:
...
def summarize(
    *,
    ...
    if (
        ...
        raise AdoptionStressError("formal topology or filesystem-only data plane failed")
    ...
    if (
        ...
        or any(not math.isfinite(float(event["token_weighted_loss"])) for event in events)
    ):
    ...
    for trace in traces:
        ...
        if (
            ...
            raise AdoptionStressError("formal parameter or optimizer identity audit failed")
    ...
    first_adoption_step = min(int(trace["safe_boundary_local_step"]) for trace in traces)
    mixed_window = [
        event
        for event in events
    ...
    if (
        ...
        or poller["resident_pending_bound"] != int(profile["training"]["fragment_count"])
...
def write_manifest(args: argparse.Namespace) -> None:
    ...
    if (
        ...
        raise AdoptionStressError("manifest role state config or run identity is invalid")
    if args.queue != "debug-g" or args.group != "xg24i002":
        raise AdoptionStressError("manifest scheduler queue or group differs from contract")
    role_map = {
        "mixed_version_learner": [learner["hostname"]],
        "global_fragment_writer": [writer["hostname"]],
    }
    if set(role_map["mixed_version_learner"] + role_map["global_fragment_writer"]) != set(
        hosts
    ):
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m fsbdd.learner_adopt_stress")
    ...
    if args.command == "role":
        ...
        print(json.dumps({"status": value["status"], "role": value["role"]}, sort_keys=True))
        return 0
