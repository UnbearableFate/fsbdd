...
class EvidencePackage:
    ...
    def bind_resume_identity(self, identity: dict[str, str]) -> None:
        ...
        if identity.keys() != needed or any(not value for value in identity.values()):
            raise EvidenceError("resume identity requires exact code/config/generator fields")
    ...
    def verify_resume_identity(self, identity: dict[str, str]) -> None:
        ...
        if not path.is_file() or json.loads(path.read_text(encoding="utf-8")) != identity:
    ...
    def finalize(self, manifest: dict[str, Any]) -> dict[str, Any]:
        ...
        manifest["evidence"] = {"files": files, "inventory_sha256": canonical_digest(files)}
        manifest["status"] = "finalized"
        validate_manifest(manifest)
        manifest_path = self.root / "manifest.json"
        manifest_path.write_text(json.dumps(manifest, sort_keys=True, indent=2) + "\n", encoding="utf-8")
        checksum_paths = ["manifest.json", *files]
        checksum_text = "".join(f"{file_digest(self.root / relative)}  {relative}\n" for relative in checksum_paths)
        ...
        (self.root / "validation-summary.json").write_text(json.dumps(summary, sort_keys=True, indent=2) + "\n", encoding="utf-8")
        return summary
...
def validate_package(root: Path) -> dict[str, Any]:
    ...
    listed = manifest.get("evidence", {}).get("files", []) if isinstance(manifest, dict) else []
    if listed != actual:
    ...
    return {
        ...
        "manifest_sha256": file_digest(root / "manifest.json") if (root / "manifest.json").is_file() else None,
    }
