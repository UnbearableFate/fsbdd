...
def run_role(
    *,
    ...
    try:
        ...
        raise RuntimeError("storage stress role requires an MPI launcher environment") from error
    ...
    if rank == 0:
        ...
        _replace_json(coordination / "writer-initial.json", {"complete": True, "sequence": 0})
        ...
        _replace_json(coordination / "writer-done.json", {"complete": True, "sequence": publications})
    ...
    while True:
        try:
            value = backend.read("latest", _expectation(run_id), timeout_seconds=5, poll_interval_seconds=0.001)
    ...
    _replace_json(coordination / "reader-done.json", {"complete": True, "sequence": publications})
    result = {
        "schema_version": 1,
        "status": "complete",
        "role": "reader",
...
def _crash_matrix(root: Path, run_id: str) -> list[dict[str, Any]]:
    ...
    for crash_at, expected_sequence in expected.items():
        ...
        try:
            backend.publish("current", b"new-compound", _spec(run_id, 1), crash_at=crash_at)
        ...
        if visible.record.sequence != expected_sequence:
            raise AssertionError(f"wrong crash outcome at {crash_at}: {visible.record.sequence}")
...
def summarize(*, root: Path, result_root: Path, run_id: str, publications: int, output: Path) -> dict[str, Any]:
    ...
    if writer.get("publications") != publications or reader.get("maximum_sequence") != publications:
    ...
    if reader.get("invalid_reads") != 0 or reader.get("errors") != []:
        raise AssertionError("cross-node reader observed an invalid or partial publication")
...
def write_manifest(args: argparse.Namespace) -> None:
    ...
    modules = [line.strip() for line in Path(args.modules_file).read_text(encoding="utf-8").splitlines() if line.strip()]
    writer = json.loads((Path(args.result_root) / "writer.json").read_text(encoding="utf-8"))
    reader = json.loads((Path(args.result_root) / "reader.json").read_text(encoding="utf-8"))
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m fsbdd.storage_stress")
    ...
    for name in (
        "repository", "branch", "commit", "config-sha256", "research-sha256", "spec-sha256",
        "skill-repository", "skill-commit", "initial-hostname", "project-root", "evidence-root",
        "job-id", "qtime-utc", "queue", "group", "nodefile", "modules-file", "result-root", "output",
    ):
    ...
    if args.command == "summarize":
        ...
        print(json.dumps({"status": summary["status"], "cross_node": summary["cross_node"]}, sort_keys=True))
        return 0
    write_manifest(args)
