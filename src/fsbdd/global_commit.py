...
class CommitEnvelope:
    ...
    def __post_init__(self) -> None:
        ...
        _require_hex(
            self.previous_authority_identity, "previous_authority_identity"
        )
        ...
        if len(set(learners)) != len(learners):
            raise AtomicCommitError("selected contributions contain a duplicate learner")
        if len(set(proposals)) != len(proposals):
            raise AtomicCommitError("selected contributions contain a duplicate proposal")
        if self.selected:
...
def encode_commit_envelope(envelope: CommitEnvelope) -> bytes:
    ...
    return _MAGIC + struct.pack(">Q", len(header)) + header + envelope.outer_optimizer_state
...
def decode_commit_envelope(
    ...
    *,
    ...
    try:
        ...
        raise AtomicCommitError("commit envelope header is not complete JSON") from error
...
class AtomicCommitRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise AtomicCommitError("selection fragment or version differs from request")
            ...
            dataclasses.replace(
                ...
                outer_state=encode_commit_envelope(
                    self._empty_envelope(
                        fragment.descriptor, fragment.outer_state
                    )
                ),
            )
            ...
            digest=canonical_digest(
                {
                    ...
                    "authorities": [
                        item.authority_identity for item in authorities
                    ],
                    "version_vector": versions,
                }
            ),
    ...
    def _validate_selection_against_authority(
        ...
        for proposal in selection.proposals:
            ...
            if proposal.learner_id in learners:
                raise AtomicCommitError("selected proposals contain a duplicate learner")
            if proposal.proposal_id in proposals:
                raise AtomicCommitError("selected proposals contain a duplicate identity")
            ...
            if proposal.sequence <= frontier.last_sequence:
                raise AtomicCommitError("selected proposal sequence is already consumed")
        ...
        *,
        ...
        if current.version == request.next_version:
            if self._matches_retry(current, request):
                return AtomicCommitResult(current, published=False, duplicate_retry=True)
            raise AtomicCommitError(
