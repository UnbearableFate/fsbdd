...
from fsbdd.auxiliary.contracts.evidence import (
    EvidenceError,
    EvidencePackage,
    validate_package,
)
from fsbdd.auxiliary.contracts.manifest import (
    ManifestError,
    build_manifest,
    validate_manifest,
)
...
def identities() -> dict[str, object]:
    return {
        ...
        "execution": {
            "identity": "local-nonce",
            "initial_hostname": "host",
            "workflow": "local",
        },
...
def test_dual_manifest_profiles_and_qtime_semantics() -> None:
    local = build_manifest(
        "S1-00", "L1", "2026-07-15T00:00:00Z", "submission_utc", identities()
    )
...
def test_dirty_placeholder_role_and_resume_identity_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(EvidenceError, match="resume"):
        package.verify_resume_identity(
            {"code": "a", "config": "changed", "generator": "c"}
        )
...
def test_identity_shape_mutations_rejected(
    section: str, mutation: dict[str, object]
) -> None:
    bad = identities()
